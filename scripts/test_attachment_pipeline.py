...
def list_all_attachments():
    ...
    try:
        ...
        for att in attachments:
            object_key = (att.object_key or "")[:28] + ".." if att.object_key and len(att.object_key) > 30 else (att.object_key or "")
            print(f"{att.id:<5} {att.lead_id:<6} {att.upload_status:<12} {att.upload_attempts:<8} {object_key:<30}")
...
if __name__ == "__main__":
    ...
    if check_attachments():
        ...
        print("   docker compose exec api python -m app.jobs.sweep_pending_uploads --limit 50 --verbose")
    else:
