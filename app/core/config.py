...
class Settings(BaseSettings):
    ...
    admin_api_key: str | None = None  # Optional - if not set, admin endpoints are unprotected (dev mode)
    ...
    google_sheets_credentials_json: str | None = None  # Path to service account JSON or JSON content

    # Action tokens (Mode B - WhatsApp action links)
    action_token_base_url: str = "http://localhost:8000"  # Base URL for action links (set in production)
